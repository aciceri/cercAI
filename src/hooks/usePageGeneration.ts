...
export function usePageGeneration(apiConfig: APIConfig | null) {
  ...
  const createCacheKey = useCallback((result: ResultPageRequest): string => {
    return btoa(
      `${result.originalQuery}-${result.result.title}-${result.result.url}`,
    );
  }, []);

  const getCachedPage = useCallback(
    (request: ResultPageRequest): GeneratedPage | null => {
  ...
  const setCachedPage = useCallback(
    (request: ResultPageRequest, page: GeneratedPage) => {
      const key = createCacheKey(request);
      setCache((prev) => ({
        ...prev,
        [key]: page,
  ...
  const generatePage = useCallback(
    async (request: ResultPageRequest): Promise<GeneratedPage> => {
      if (!apiConfig) {
  ...
  const loadPage = useCallback(
    async (request: ResultPageRequest): Promise<void> => {
      // Check cache first
